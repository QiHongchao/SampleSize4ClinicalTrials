...
#' @return samplesize
#'
#' @usage ssc_meancomp(design, ratio, alpha, power, sigma, theta, delta)
#'
...
ssc_meancomp<-function(design = c(1L,2L,3L,4L), ratio = 1, alpha = 0.05, power = 0.8, sigma = NULL,theta = NULL, delta = NULL){
  if (!(design %in% 1L:4L))
  ...
  if (design == 4L) {
    numerator <- (abs(qnorm(alpha)) + abs(qnorm((1 - power)/2)))^2 * sigma * (1 + 1/ratio)
  }
