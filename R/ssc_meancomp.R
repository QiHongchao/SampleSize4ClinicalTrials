...
#' @param var The variance of observed outcomes in both arms
...
#' @return samplesize
#'
#' @usage ssc_meancomp(design, ratio, alpha, power, var, theta, delta)
#'
...
ssc_meancomp<-function(design = c(1L,2L,3L,4L), ratio = 1, alpha = 0.05, power = 0.8, var = NULL,theta = NULL, delta = NULL){
  if (!(design %in% 1L:4L))
    stop("Unrecognized study design, 1: Test for equility, 2: Superiority trial,
         3: Non-inferiority trial, 4: Equivalence trial")
  ##Numerator
  if (design==1L) {
    numerator <- (abs(qnorm(alpha/2)) + abs(qnorm(1- power)))^2 * var * (1 + 1/ratio)
  }
  ...
  if (design %in% 2L:3L) {
    numerator <- (abs(qnorm(alpha)) + abs(qnorm(1 - power)))^2 * var * (1 + 1/ratio)
  }
  if (design == 4L) {
    numerator <- (abs(qnorm(alpha)) + abs(qnorm((1 - power)/2)))^2 * var * (1 + 1/ratio)
  }
